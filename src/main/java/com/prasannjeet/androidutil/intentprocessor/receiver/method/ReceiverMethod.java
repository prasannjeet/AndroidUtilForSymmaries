...
public class ReceiverMethod {
  ...
  private ReceiverMethod(boolean isStatic, List<IntentCallingMethod> intentCallingMethods) {
    ...
    this.methodBlocks.setInitialDeclaration(
        (isStatic ? "static " : "")
            + "void "
            + FULL_CLASS_NAME
            + ":"
    ...
    this.methodBlocks.setIntentActionStatement(
        intentActionVariable + " = " + this.intentVariable + ".getAction();");
  ...
  private void processIntentCallingMethod(IntentCallingMethod icm) {
    ...
    for (String acceptedIntent : icm.getAcceptedIntentTypes()) {
      this.methodBlocks.addIfBlock(
          createOneIfBlock(acceptedIntent, methodClassName, icm.getMethodName(), methodArgs));
    }
  }

  private String createOneIfBlock(
      String intentType, String methodClassName, String methodName, List<String> methodArgs) {
    String l1 =
        "label"
            + this.labelCount
            + ":"
            + this.boolConditionVariable
            + " = "
            + this.intentActionVariable
            + ".equals((java.lang.Object)\"android.intent.action."
            + intentType
            + "\");";
